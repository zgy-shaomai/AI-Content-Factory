...
很多人第一次看这个项目，会误以为系统只有一次模型调用，例如“把商品资料扔给 AI，然后自动吐出图片和视频”。实际上不是。这里是**多个模型在不同阶段分工协作**，中间还穿插了大量非模型步骤，比如数据库读写、状态推进、审核回写、文件归档和 FFmpeg 合成。

### 3.1 总体分工

| 阶段 | 是否模型介入 | 负责者 | 作用 |
|---|---|---|---|
| 商品资料录入 | 否 | 飞书 / Webhook / n8n | 接收任务 |
| 商品属性解析 | 是 | Claude Sonnet 4.6 | 把原始资料整理成结构化信息 |
| 风格快照读取 | 否 | PostgreSQL / n8n | 读取品牌风格模板 |
| 图片 prompt 生成 | 是 | Claude Sonnet 4.6 | 生成图片提示词 |
| 图片实际生成 | 是 | Seedream 4.0 | 生成候选图片 |
| 图片评分 / QC | 是 | Claude Sonnet 4.6 多模态 | 对候选图做一致性和质量评分 |
| 视频分镜脚本生成 | 是 | Claude Sonnet 4.6 | 生成 4 镜脚本 |
| 视频关键帧生成 | 是 | Seedream 4.0 | 生成每镜首帧 |
| 视频 prompt 生成 | 是 | Claude Sonnet 4.6 | 生成视频提示词 |
| 视频实际生成 | 是 | Seedance 2.0 | 生成视频分镜 |
| 字幕转写 | 是 | ASR 服务 | 音频转字幕 |
| 视频拼接 / 烧录字幕 | 否 | FFmpeg / n8n | 产出最终成片 |
| 审核回写 / 归档 | 否 | 飞书 / PostgreSQL / OSS / n8n | 记录与交付 |

### 3.2 文本模型负责什么

本项目里的文字模型主要是 Claude Sonnet 4.6，经由 5dock NewAPI 调用。

它只负责“理解”和“组织”，不负责真正生成媒体文件。它的典型职责是：

1. 把原始商品资料整理成结构化字段
2. 把卖点翻译成适合视觉表达的要点
3. 把结构化字段和风格模板组装成图片 prompt
4. 把商品资料改写成视频分镜脚本
5. 把分镜脚本再改写成视频 prompt
6. 在审核打回时，根据反馈修订 prompt

所以它本质上是这个系统里的“文案理解器 + 提示词生成器 + 改写器”。

### 3.3 图像模型负责什么

图像模型这里主要是 Seedream 4.0，经由火山方舟调用。

它负责两类输出：

1. 图片链路里的商品图、场景图、细节图
2. 视频链路里的关键帧图，也就是后续 Seedance 的首帧输入

也就是说，Seedream 既服务于“静态图产出”，也服务于“视频前置锚点生成”。

### 3.4 视频模型负责什么

视频模型这里主要是 Seedance 2.0，经由火山方舟调用。

它不是直接从商品资料生成整条 12 秒视频，而是：

1. 接收某一镜的首帧图
2. 接收某一镜的视频 prompt
3. 生成一个 3 秒左右的视频片段

四镜都生成后，再由本地工作流拼接成最终成片。

因此 Seedance 的职责是“逐镜生成动态片段”，而不是“独立完成从脚本到最终交付的全部流程”。
...
- 飞书 / Web 表单负责录入
- PostgreSQL 负责状态、任务、候选、归档元数据
- n8n 负责编排、重试、分支流转、回写
- OSS 负责存储资产
- FFmpeg 负责拼接视频与烧录字幕
- 审核员负责最终通过 / 打回决策

所以这个项目不是“纯模型项目”，而是“模型嵌入到工程流水线中的项目”。

---

## 4. 数据流详解：YN-BRA-001 全链路 13 步
...
### 7.2 资源开销估算（2C4G VPS · Ubuntu 22.04）
...
下面这些事项建议在正式上线前确认，否则容易卡在部署或验收阶段。
...
| 10 | 保修期范围 | 1 个月内：bug 修复、参数微调；不含新需求 | 需单独确认 |
