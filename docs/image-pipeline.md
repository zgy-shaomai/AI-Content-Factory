...
## 3. Seedream 4.0 调用参数策略

### 3.1 Body 模板（POST /api/v3/images/generations）
...
### 3.5 reference_images（image 数组）传法
...
## 5. 候选图 metadata（写回 candidates 表）
...
完整链路可在客户面前 5 分钟内出第一波结果，10 分钟内完成"提交 → 看图 → 审核 → 二次生成 → 看到改进"的完整闭环演示。
