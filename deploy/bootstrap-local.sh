...
for i in $(seq 1 30); do
    if docker exec cf-postgres-local pg_isready -U postgres >/dev/null 2>&1; then
...
COUNT=$(docker exec cf-postgres-local psql -U postgres -d content_factory -tAc \
...
ENUM_OK=$(docker exec cf-postgres-local psql -U postgres -d content_factory -tAc \
...
${GRN}✅ 本地栈起完了${NC}
  ...
  Postgres:    localhost:55432  user=postgres  db=content_factory
...
测试触发：
  ...
  从 PG 查 task uuid:
  docker exec cf-postgres-local psql -U postgres -d content_factory \\
